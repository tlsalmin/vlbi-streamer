...
#define POINT_TO_THE_RIGHT_DIRECTION cce->buffer+cce->offset+cce->netoffset
...
void mark5b_metadata_increment(struct common_control_element* cce, long count)
{
  ...
  */
  (void)count;
  if(!(syncword_check(cce->buffer+cce->netoffset) == 0))
  {
    struct metadata_mark5b *ms = cce->datatype_metadata;
...
{
  struct metadata_mark5b* ms = cce->datatype_metadata;
  cce->netoffset = BITUP(cce->optbits, DATATYPE_MARK5BNET) ? 8 : 0;
  if(syncword_check(cce->buffer+cce->netoffset) == 0)
  {
...
void mark5_printcommand(struct metadata_mark5b* ms)
{
  if(ms->m5netframenum != -1)
    LOG("| M5NetFrameNum: %ld ", ms->m5netframenum);
...
int mark5_printmetadata(struct common_control_element *cce)
{
  ...
  if((cce->optbits & DATATYPE_MARK5BNET) && (syncword_check(POINT_TO_THE_RIGHT_DIRECTION) == 0))
  {
    /* Not printing between-packets	*/
    return 0;
  }
  //copy_m5metadata(cce, ms);
...
int mark5b_discrepancy(struct common_control_element *cce)
{
  ...
  if(syncword_check(POINT_TO_THE_RIGHT_DIRECTION) == 0)
  {
    if(cce->optbits & DATATYPE_MARK5BNET)
    {
      /* Only check m5netframenum on between packets	*/
      if(((long)NETFRAMENUM_FROM_MARK5BNET(cce->buffer+cce->offset)) != prev_ms->m5netframenum+1)
      {
	LOG("Inconsistency as mark5bnet framenum was %ld and now is %d\n", prev_ms->m5netframenum, NETFRAMENUM_FROM_MARK5BNET(cce->buffer+cce->offset));
	//LOG_DEFAULT_BROKEN_PACKET;
      }
    }
    /* The else here would be mark5b normal data split to 5008 byte packets	*/
    /* This isn't specced anywhere so no extra steps taken			*/
  }
  else
  {
    ...
    if(cce->optbits & DATATYPE_MARK5BNET)
    {
      ...
      {
	LOG("Inconsistency as mark5bnet framenum was %ld and now is %ld\n", prev_ms->m5netframenum, ms->m5netframenum);
	LOG_DEFAULT_BROKEN_PACKET;
      }
    }
