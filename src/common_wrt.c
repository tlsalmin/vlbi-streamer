...
void common_infostring(void *le, char *returnable)
{
  ...
  struct common_io_info *ioi = (struct common_io_info *)re->opt;
  sprintf(returnable, "%s%i%s%s%s%lu", "ID: ", ioi->id, " Filename: ",
          ioi->curfilename, " File seqnum: ", ioi->file_seqnum);
}

int check_if_suitable(void *le, void *other)
{
  struct opt_s *opt_other = (struct opt_s *)other;
  struct common_io_info *ioi =
    (struct common_io_info *)(((struct recording_entity *)le)->opt);
...
{
  return ((struct common_io_info *)((struct recording_entity *)re)->opt)->
    filesize;
}
...
void common_init_common_functions(struct opt_s *opt,
