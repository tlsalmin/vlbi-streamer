...
int sbuf_check(struct buffer_entity *be, int tout)
{
  ...
  DD("Checking for ready writes. asyndiff: %ld bytes, diff: %ld bytes",
  ...
  if (sbuf->opt->optbits & USE_RX_RING)
    {
      ...
      be->buffer =
        sbuf->opt->buffer +
        CALC_BUFSIZE_FROM_OPT(sbuf->opt) * ((long unsigned)rxr->bufnum);
    }
...
int sbuf_init(struct opt_s *opt, struct buffer_entity *be)
{
  ...
  int err;
  struct simplebuf *sbuf =
    (struct simplebuf *)malloc(sizeof(struct simplebuf));
  ...
  struct listed_entity *le =
    (struct listed_entity *)malloc(sizeof(struct listed_entity));
      ...
      if (sbuf->optbits & USE_HUGEPAGE)
        {
...
#ifdef MMAP_NOT_SHMGET
          be->buffer =
            mmap(NULL, hog_memory, PROT_READ | PROT_WRITE,
                 MAP_ANONYMOUS | MAP_PRIVATE | MAP_HUGETLB | MAP_NORESERVE,
                 -1, 0);
          ...
          sbuf->shmid =
            shm_open(shmstring, (O_CREAT | O_RDWR), (S_IREAD | S_IWRITE));
          ...
          be->buffer =
            mmap(NULL, hog_memory, (PROT_READ | PROT_WRITE),
                 MAP_ANONYMOUS | MAP_SHARED | MAP_HUGETLB | MAP_NORESERVE,
                 sbuf->shmid, 0);
...
#endif /* HAVE_HUGEPAGES */
        {
          ...
          if (hog_memory > (long unsigned)maxmem)
            {
              LOG
                ("Max allocatable memory %ld MB. still reserving %lu MB more\n",
                 maxmem / MEG, hog_memory / MEG);
              //return -1;
            }
          err =
            posix_memalign((void **)&(be->buffer), sysconf(_SC_PAGESIZE),
                           hog_memory);
...
void close_recer(struct buffer_entity *be, int errornum)
{
  ...
  if (sbuf->opt->optbits & READMODE)
    {
      /*
         afi_mark_recid_missing(sbuf->opt, sbuf->fh->diskid);
       */
...
int simple_end_transaction(struct buffer_entity *be)
{
  ...
  if (ret == EOF)
    ...
    {
      E("Error in Rec entity write: %ld. Left to write:%ld bytes", ret,
        sbuf->diff);
      close_recer(be, ret);
      return -1;
    }
...
int simple_write_bytes(struct buffer_entity *be)
{
  ...
  if (limit != count && WRITEND_USES_DIRECTIO(sbuf->opt))
    {
      D("Only need to finish transaction. limit %lu, count %lu", limit,
        count);
  ...
  while (sbuf->asyncdiff > 0)
    {
      if (sbuf->diff > 0)
        ...
        {
          DD("Only checking. Not writing. asyncdiff still %ld bytes",
             sbuf->asyncdiff);
          err = sbuf_check(be, 1);
  ...
  while (sbuf->running == 1)
    {
      ...
      if (sbuf->diff > 0)
        {
          D("Blocking reads/writes. Left to read/write %ld for file %lu",
            sbuf->diff, sbuf->fileid);
          ASSERT((unsigned)sbuf->diff <=
                 CALC_BUFSIZE_FROM_OPT_NOOFFSET(sbuf->opt));
          savedif = sbuf->diff;
          ret = -1;
          ...
          while (ret != 0 && sbuf->running == 1)
            {
              ...
              /*If everything went great */
              else
                {
                  ...
                  if (sbuf->opt->optbits & READMODE)
                    {
                      afi_update_fh(sbuf->opt->fi, sbuf->fileid,
                                               FH_INMEM, AFI_ADD_TO_STATUS);
                  ...
                  else
                    {
                      ...
                      afi_update_fh(sbuf->opt->fi, sbuf->fileid,
                                               FH_BUSY, AFI_DEL_FROM_STATUS);
                      afi_update_fh(sbuf->opt->fi, sbuf->fileid,
                                               FH_ONDISK, AFI_ADD_TO_STATUS);
                      if (afi_wake_up(sbuf->opt->fi) != 0)
